...
%   RADGRAD(SIZE, POINT, RADIUS, COLORS, {METHOD}, {BREAKPOINTS}, {OUTCLASS}, {COLORTYPE})
...
%
% Webdocs: http://mimtdocs.rf.gd/manual/html/radgrad.html
% See also: lingrad
...
switch outclass
	case 'uint8'
		...
		if mx > 255 || mn < 0
			error('RADGRAD: numeric range of COLORS is inappropriate for image class ''uint8''');
		elseif mx <= 1
			disp('RADGRAD: numeric range of COLORS is [0 1] for image class ''uint8'', was this intended?');
		end
	case 'uint16'
		[mn mx] = imrange(colors);
		if mx > 65535 || mn < 0
			error('RADGRAD: numeric range of COLORS is inappropriate for image class ''uint16''');
		elseif mx <= 1
			disp('RADGRAD: numeric range of COLORS is [0 1] for image class ''uint16'', was this intended?');
		end
	case {'single','double'}
		[mn mx] = imrange(colors);
		if mx > 65535 || mn < 0
			error('RADGRAD: numeric range of COLORS is inappropriate for floating point image class');
		elseif mx > 1.5 && mx <= 255
			colors = colors./255;
			disp('RADGRAD: numeric range of COLORS assumed to be [0 255]');
		elseif mx > 1.5 && mx <= 65535
			colors = colors./65535;
			disp('RADGRAD: numeric range of COLORS assumed to be [0 65535]');
		end
end
...
switch lower(colortype)
	...
	otherwise
		error('RADGRAD: supported values for COLORTYPE are ''srgb'' and ''linrgb''')
end
