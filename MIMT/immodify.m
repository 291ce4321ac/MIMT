...
if ~isempty(h)
	...
	lvvec = [0 1 0 1 1 1 1 1]; % [[in] [out] g sg rg k]
	gammamax = 2.5;
	rgammamax = 2.5;
	sgammamax = 2.5;
	contrastmax = 2.5;
...
function figuresetup()
	...
	vp = 0.01*vertscale;
	hm = 0.05;
	vm = 0.27; % control stack origin (above axes)
	editbxgeo = [0.30 bh];
	slidergeo = [1-2*hm bh];
	bkspy = vp+lh+2*bh;
	bkor = vm+2*vp;
	...
	'Units','normalized',...
	'FontSize',10,...
	'Position',[hm bkor+7*bkspy+2*lh+bh+2*vp 0.20 bh],...
	'String','Auto',...
	...
	'Units','normalized',...
	'FontSize',10,...
	'Position',[hm+0.2 bkor+7*bkspy+2*lh+bh+2*vp 0.20 bh],...
	'String','Reset',...
	...
	'Parent',ep2,...
	'Units','normalized',...
	'Position',[0.50 bkor+7*bkspy+2*lh+bh+2*vp 0.45 bh],...
	...
	editboxparams = {'Parent',ep2,...
					...
					'Tag','lvbox'};
	slabelparams = {'Parent',ep2,...
					'Units','normalized',...
					'FontSize',8,...
					'HorizontalAlignment','left',...
					'Style','text',...
					'Tag','levellabel'};
	sliderparams = {'Parent',ep2,...
					'Units','normalized',...
					'BackgroundColor',[0.9 0.9 0.9],...
					'Style','slider',...
					'Tag','lvslider'};
	minlabelparams = {'Parent',ep2,...
					'Units','normalized',...
					'FontSize',8,...
					'HorizontalAlignment','left',...
					'String','0',...
	...
	% adding color helps visually group related controls
	% i hate doing it like this, but things are getting too tight
	if invert
		CT = [0.95066 0.7809 0.86805; 0.86928 0.82913 0.75324; 0.72031 0.89737 0.7995; 0.68325 0.87767 1];
	else
		CT = [0.70196 0.88627 0.80392; 0.79608 0.83529 0.9098; 0.95686 0.79216 0.89412; 0.99216 0.80392 0.67451];
	end
	
	% slider 1
	uicontrol(editboxparams{:},...
	'Position',[0.65 bkor+7*bkspy+lh+bh editbxgeo],...
	'String',lvvec(1),...
	'callback',{@setparam,'levelval',1});
	uicontrol(slabelparams{:},...
	'Position',[0.108949-0.05 bkor+7*bkspy+lh+bh 0.354 lh],...
	'String','Input Min');
	uicontrol(sliderparams{:},...
	'backgroundcolor',CT(1,:),...
	'Position',[hm bkor+7*bkspy+lh slidergeo],...
	'Value',lvvec(1),...
	'callback',{@setparam,'levelslider',1});
	uicontrol(minlabelparams{:},...
	'Position',[0.10 bkor+7*bkspy 0.2 lh]);
	uicontrol(maxlabelparams{:},...
	'Position',[0.78 bkor+7*bkspy 0.2 lh]);

	% slider 2
	uicontrol(editboxparams{:},...
	'Position',[0.65 bkor+6*bkspy+lh+bh editbxgeo],...
	'String',lvvec(2),...
	'callback',{@setparam,'levelval',2});
	uicontrol(slabelparams{:},...
	'Position',[0.108949-0.05 bkor+6*bkspy+lh+bh 0.354 lh],...
	'String','Input Max');
	uicontrol(sliderparams{:},...
	'backgroundcolor',CT(1,:),...
	'Position',[hm bkor+6*bkspy+lh slidergeo],...
	'Value',lvvec(2),...
	'callback',{@setparam,'levelslider',2});
	uicontrol(minlabelparams{:},...
	'Position',[0.10 bkor+6*bkspy 0.2 lh]);
	uicontrol(maxlabelparams{:},...
	'Position',[0.78 bkor+6*bkspy 0.2 lh]);

	% slider 3
	uicontrol(editboxparams{:},...
	'Position',[0.65 bkor+5*bkspy+lh+bh editbxgeo],...
	'String',lvvec(3),...
	'callback',{@setparam,'levelval',3});
	uicontrol(slabelparams{:},...
	'Position',[0.108949-0.05 bkor+5*bkspy+lh+bh 0.354 lh],...
	'String','Output Min');
	uicontrol(sliderparams{:},...
	'backgroundcolor',CT(2,:),...
	'Position',[hm bkor+5*bkspy+lh slidergeo],...
	'Value',lvvec(3),...
	'callback',{@setparam,'levelslider',3});
	uicontrol(minlabelparams{:},...
	'Position',[0.10 bkor+5*bkspy 0.2 lh]);
	uicontrol(maxlabelparams{:},...
	'Position',[0.78 bkor+5*bkspy 0.2 lh]);

	% slider 4
	uicontrol(editboxparams{:},...
	'Position',[0.65 bkor+4*bkspy+lh+bh editbxgeo],...
	'String',lvvec(4),...
	'callback',{@setparam,'levelval',4});
	uicontrol(slabelparams{:},...
	'Position',[0.108949-0.05 bkor+4*bkspy+lh+bh 0.354 lh],...
	'String','Output Max');
	uicontrol(sliderparams{:},...
	'backgroundcolor',CT(2,:),...
	'Position',[hm bkor+4*bkspy+lh slidergeo],...
	'Value',lvvec(4),...
	'callback',{@setparam,'levelslider',4});
	uicontrol(minlabelparams{:},...
	'Position',[0.10 bkor+4*bkspy 0.2 lh]);
	uicontrol(maxlabelparams{:},...
	'Position',[0.78 bkor+4*bkspy 0.2 lh]);

	% slider 5
	uicontrol(editboxparams{:},...
	'Position',[0.65 bkor+3*bkspy+lh+bh editbxgeo],...
	'String',lvvec(5),...
	'callback',{@setparam,'levelval',5});
	uicontrol(slabelparams{:},...
	'Position',[0.108949-0.05 bkor+3*bkspy+lh+bh 0.354 lh],...
	'String','Gamma');
	uicontrol(sliderparams{:},...
	'backgroundcolor',CT(3,:),...
	'Position',[hm bkor+3*bkspy+lh slidergeo],...
	'Value',lvvec(5),...
	'max',gammamax,...
	'callback',{@setparam,'levelslider',5});
	uicontrol(minlabelparams{:},...
	'Position',[0.10 bkor+3*bkspy 0.2 lh]);
	uicontrol(maxboxparams{:},...
	'Position',[0.78 bkor+3*bkspy 0.15 lh],...
	'String',gammamax,...
	'Tag','gammamaxbox',...
	...
	% slider 6
	uicontrol(editboxparams{:},...
	'Position',[0.65 bkor+2*bkspy+lh+bh editbxgeo],...
	'String',lvvec(6),...
	'callback',{@setparam,'levelval',6});
	uicontrol(slabelparams{:},...
	'Position',[0.108949-0.05 bkor+2*bkspy+lh+bh 0.354 lh],...
	'String','SymGamma');
	uicontrol(sliderparams{:},...
	'backgroundcolor',CT(3,:),...
	'Position',[hm bkor+2*bkspy+lh slidergeo],...
	'Value',lvvec(6),...
	'max',sgammamax,...
	'callback',{@setparam,'levelslider',6});
	uicontrol(minlabelparams{:},...
	'Position',[0.10 bkor+2*bkspy 0.2 lh]);
	uicontrol(maxboxparams{:},...
	'Position',[0.78 bkor+2*bkspy 0.15 lh],...
	'String',sgammamax,...
	'Tag','sgammamaxbox',...
	'callback',{@setparam,'sgammamax'});

	% slider 7
	uicontrol(editboxparams{:},...
	'Position',[0.65 bkor+bkspy+lh+bh editbxgeo],...
	'String',lvvec(7),...
	'callback',{@setparam,'levelval',7});
	uicontrol(slabelparams{:},...
	'Position',[0.108949-0.05 bkor+bkspy+lh+bh 0.354 lh],...
	'String','RevGamma');
	uicontrol(sliderparams{:},...
	'backgroundcolor',CT(3,:),...
	'Position',[hm bkor+bkspy+lh slidergeo],...
	'Value',lvvec(7),...
	'max',rgammamax,...
	'callback',{@setparam,'levelslider',7});
	uicontrol(minlabelparams{:},...
	'Position',[0.10 bkor+bkspy 0.2 lh]);
	uicontrol(maxboxparams{:},...
	'Position',[0.78 bkor+bkspy 0.15 lh],...
	'String',rgammamax,...
	'Tag','rgammamaxbox',...
	'callback',{@setparam,'rgammamax'});

	% slider 8
	uicontrol(editboxparams{:},...
	'Position',[0.65 bkor+lh+bh editbxgeo],...
	'String',lvvec(8),...
	'callback',{@setparam,'levelval',8});
	uicontrol(slabelparams{:},...
	'Position',[0.108949-0.05 bkor+lh+bh 0.354 lh],...
	'String','Contrast');
	uicontrol(sliderparams{:},...
	'backgroundcolor',CT(4,:),...
	'Position',[hm bkor+lh slidergeo],...
	'Value',lvvec(8),...
	'max',contrastmax,...
	'callback',{@setparam,'levelslider',8});
	uicontrol(minlabelparams{:},...
	'Position',[0.10 bkor 0.2 lh]);
	uicontrol(maxboxparams{:},...
	'Position',[0.78 bkor 0.15 lh],...
	'String',contrastmax,...
	'Tag','contrastmaxbox',...
...
function setparam(objh,~,param,idx)
	switch param			
		...
		case 'sgammamax'
			sgammamax = abs(str2double(get(objh,'string')));
			updatelvsliders(6)
			
		case 'rgammamax'
			rgammamax = abs(str2double(get(objh,'string')));
			updatelvsliders(7)
...
function resetparams()
	...
	elseif (any(transition == 3) && any(transition ~= 3)) ...
		|| any(transition == 4) && any(transition ~= 4)
		% if switching between a rectangular mode and any mode of a differing type
		cvec = [1 1 1; 0 0 0];
		cvecmax = [cmax*[1 1 1]; 1 1 1];
		cvecinv = [0 0 0; 0 0 0];
	else
		% keep vectors when switching between modes of similar geometry type
	end
...
function updatelvsliders(channel)
	...
	elseif channel == 6
		set(handles.lvslider(channel),'max',sgammamax);
		set(handles.sgammamaxbox,'string',num2str(sgammamax));
	elseif channel == 7
		set(handles.lvslider(channel),'max',rgammamax);
		set(handles.rgammamaxbox,'string',num2str(rgammamax));
	elseif channel == 8
	...
	hd = plot([0 1],[0 1],'parent',handles.imadjustaxes); hold on;
	set(hd,'color',[1 1 1]*0.8);
 	a = bar(handles.imadjustaxes,bins,hc/max(hc),'hist');
	set(a,'edgealpha',0,'facealpha',0.25);	
	
	inc = 0:0.01:1;
	outc = imlnc(inc,'in',lvvec(1:2),'out',lvvec(3:4),'g',lvvec(5),'sg',lvvec(6),'rg',lvvec(7),'k',lvvec(8));
	hl = plot(inc,outc,'parent',handles.imadjustaxes); 
	set(hl,'color',[0 0.447 0.741]);
	...
	axis(handles.imadjustaxes,'square')
end
...
function updatehistogram()
	...
	bins = 0:0.05:1;
	hc = histc(reshape(sample,1,[]),bins); %#ok<HISTC>
...
function previewbutton_CBF(~,~)
	...
	switch mode
		...
		case 'levels'
			mode = imadjustmodestrings{selectedimadjustmode};
			mode = lower(regexprep(mode,'[\(\)]',''));
			if strismember(mode,{'i','rgb'})
				mode = 'independent';
			end
			protoimage = imlnc(inpict,mode,'in',lvvec(1:2),'out',lvvec(3:4),'g',lvvec(5),...
								'sg',lvvec(6),'rg',lvvec(7),'k',lvvec(8));
